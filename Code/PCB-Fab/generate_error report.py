...
def parse_drc_violations(json_file: str) -> List[Tuple[float, float, int, int, str]]:
    """
    ...
    Returns:
        List[Tuple]: List of (x, y, id, sub_id, description) tuples for each violation
    """
    ...
    for violation in data.get("violations", []):
        ...
        for item in violation.get("items", []):
            ...
            if "x" in pos and "y" in pos:
                violations.append((float(pos["x"]), float(pos["y"]),
                                   idx, sub_id,
...
def add_arrows_to_svg(svg_file: str, violations: List[Tuple[float, float, int, int, str]], output_svg_file: str) -> str:
    """
    Add red arrows to an SVG file at the specified positions
    
    Args:
        svg_file (str): Path to the original SVG file
        violations (List[Tuple]): List of (x, y, id, sub_id, description) tuples
    ...
    for (x, y, id, sub_id, description) in violations:
        # Create a circle element behind the arrow and number
        circle = SubElement(drc_group, f'{{{SVG_NAMESPACE}}}circle', {
            'id': f'circle-{id}-{sub_id}',
            'cx': str(x),
            'cy': str(y),
            'r': '0.75mm',
            'fill': 'yellow',
        ...
        arrow = SubElement(drc_group, f'{{{SVG_NAMESPACE}}}text', {
            ...
            'transform': 'translate(0.05, 0)'  # Shift the arrow right by 0.05mm
        })
        arrow.text = "↓"
...
def create_html_report(svg_content: str, violations: List[Tuple[float, float, int, int, str]], output_html_file: str):
    """
    ...
    <head>
        ...
        <script>
            function highlightArrow(arrowId, circleId) {{
                document.getElementById(arrowId).classList.add('highlight-arrow');
                document.getElementById(circleId).style.visibility = 'visible';
            }}
            function unhighlightArrow(arrowId, circleId) {{
                document.getElementById(arrowId).classList.remove('highlight-arrow');
    ...
    for (_, _, id, sub_id, description) in violations:
        row_label = f"{id}" if sub_id == 1 else ""
        html_content += f"""
                <tr id="row-{id}-{sub_id}" onmouseover="highlightArrow('arrow-{id}-{sub_id}', 'circle-{id}-{sub_id}')" onmouseout="unhighlightArrow('arrow-{id}-{sub_id}', 'circle-{id}-{sub_id}')">
                    <td>{row_label}</td>
                    <td>{description}</td>
                </tr>
        """
...
def add_drc_markers(json_file: str, svg_file: str, output_svg_file: str, output_html_file: str):
    ...
    # Step 2: Add arrows to the SVG file and get the modified SVG content
    svg_content = add_arrows_to_svg(svg_file, violations, output_svg_file)
    
    # Step 3: Create the HTML report
    create_html_report(svg_content, violations, output_html_file)
