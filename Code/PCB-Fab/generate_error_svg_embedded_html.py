...
def add_arrows_to_svg(svg_file: str, violations: List[Tuple[float, float, int, str]], output_svg_file: str) -> str:
    """
    ...
    for (x, y, idx, description) in violations:
        ...
        arrow = SubElement(drc_group, f'{{{SVG_NAMESPACE}}}text', {
            ...
            'onmouseover': f"highlightRow('row-{idx}')",
            'onmouseout': f"unhighlightRow('row-{idx}')"
        })
        arrow.text = "↓"
...
def create_html_report(svg_content: str, violations: List[Tuple[float, float, int, str]], output_html_file: str):
    """
